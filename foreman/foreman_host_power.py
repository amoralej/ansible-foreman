...
description:
   - Applies a power action to a foreman host
     This only can be applied to hosts that support power actions, as managed through
     compute resources or bare metal with IPMI configured in foreman
options:
   url:
     description:
...
def _power_action(module,foreman_client):
        host_id = get_single_id_from_name('hosts',module.params['name'],module,foreman_client)
        host = foreman_client.do_put('/api/hosts/' + str(host_id) + '/power?power_action=' + module.params['power_action'], '')
...
def main():
    argument_spec = dict(
        ...
        power_action                    = dict(required=True, choices=['start', 'stop', 'poweroff', 'reboot', 'reset', 'state', 'on', 'off', 'soft', 'cycle', 'status', 'ready?']),
    )
    module = AnsibleModule(argument_spec)

    if not HAS_REQS:
        module.fail_json(msg='python-foreman and retrying are required for this module')
